...
export default function App() {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      // Transfer ownership to worker (avoids copying large buffers)
      await openDb(buffer)
  ...
  if (phase === 'ready') {
    return (
      <ArchiveContext.Provider value={archiveData}>
        <ArchiveApp onReopen={() => setPhase('pick')} />
      </ArchiveContext.Provider>
    )
  }

  return <FilePicker onFile={handleFile} loading={phase === 'loading'} error={error} />
}

function FilePicker({ onFile, loading, error }) {
    ...
    <div className="h-full flex items-center justify-center bg-slate-900">
      <div className="text-center space-y-6 max-w-sm px-6">
        <div>
          ...
          <div className="text-sm text-slate-400">Browser-native viewer — no server required</div>
        </div>

        <label
          onDrop={handleDrop}
          onDragOver={e => e.preventDefault()}
          className="block border-2 border-dashed border-slate-600 hover:border-slate-400 rounded-2xl p-10 cursor-pointer transition-colors"
        >
          {loading ? (
            <div className="text-slate-400 text-sm">Loading archive…</div>
          ) : (
            <>
              <div className="text-4xl mb-3">📂</div>
              <div className="text-white font-medium text-sm">Drop your archive here</div>
        ...
        {error && (
          <div className="text-red-400 text-sm bg-red-900/30 rounded-lg px-4 py-3">
            {error}
          </div>
        )}

        <div className="text-xs text-slate-600">
          The file is opened locally — nothing is uploaded.
        </div>
      </div>
    </div>
  )
